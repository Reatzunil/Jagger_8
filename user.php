...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($email) || empty($number) || empty($availability) || empty($time) || empty($services) || empty($file_name)) {
        $message = "Error: Please fill in all required fields.";
    } else {
        // Prepare and execute SQL statement using prepared statements
        $stmt = $conn->prepare("INSERT INTO reservations (name, email, number, availability, time, services, file_name) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssss", $name, $email, $number, $availability, $time, $services, $file_name);
